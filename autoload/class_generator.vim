...
function! class_generator#TemplatesCompletion(argLead, command, cursorPos)
  let result = []
...
function! class_generator#Completion(argLead, command, cursorPos)
  let result = []
  let commandTokens = split(a:command, ':', 1)
  let command = len(commandTokens) >= 1 ? commandTokens[-1] : a:command
  if command[0] == '/'
...
function! class_generator#CreateInFile()
...
function! class_generator#CreateClass()
  let message = "enter new class name: "
  let userinput = input(message, '', 'customlist,class_generator#Completion')
  if empty(userinput)
    return
  endif
...
function! s:CreateClass(data)
  ...
  if (fileSize <= 0 && fileSize > -2) || (line('$') == 1 && getline(1) == '')
    ...
    if has_key(a:data, 'template')
      ...
      call generators#GenerateClass(options, a:data['template'])
    else
      call generators#GenerateClass(options)
    endif
