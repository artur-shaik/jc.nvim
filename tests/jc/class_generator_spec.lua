describe("class_generator parsing", function()
  ...
  describe("parse_input", function()
    ...
    it("a capitalised word + colon is the class with flags, not a template", function()
      local p = cg.parse_input("Foo:constructor")
      assert.is_nil(p.template)
      assert.are.equal("Foo", p.path_str)
      assert.are.equal(":constructor", p.flags)
    end)
...
describe("template prefix vs class name", function()
  local cg = require("jc.class_generator")

  it("keeps a capitalised word as the class, not a template", function()
    local p = cg.parse_input("RefundRequestResultDto1:lombokGetter")
    assert.is_nil(p.template)
    assert.are.equal("RefundRequestResultDto1", p.path_str)
    assert.are.equal(":lombokGetter", p.flags)
  end)

  it("still reads a real template prefix", function()
    local p = cg.parse_input("service:/com.app.Foo")
    assert.are.equal("service", p.template)
    assert.are.equal("/com.app.Foo", p.path_str)

    local q = cg.parse_input("entity:/p.Order(String n):toString")
    assert.are.equal("entity", q.template)
    assert.are.equal("(String n)", q.fields_str)
    assert.are.equal(":toString", q.flags)
  end)

  it("does not mistake an unknown word for a template", function()
    local p = cg.parse_input("Foo:lombokData:toString")
    assert.is_nil(p.template)
    assert.are.equal("Foo", p.path_str)
